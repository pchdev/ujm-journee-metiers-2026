process = vgroup("Birds",environment{declare name "bird";
...
autoTrig = ba.beat(t) * (abs(no.noise) <= p) : trigger(48) with {
    t = hslider("[1]Speed (Granulator)[style:knob][acc:0 1 -10 0 10]", 240, 120, 480, 0.1) 
      : si.smooth(0.999);
    p = hslider("[2]Probability (Granulator)[unit:%][style:knob][acc:1 0 -10 0 10]", 50, 25, 100, 1)*(0.01) 
      : si.smooth(0.999);
